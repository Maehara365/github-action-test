...
// console.log(chalk.blue("basename:", path.basename("./dir/test.txt")));
// console.log(chalk.blue("dirname:", path.dirname("./dir/test.txt")));
...
// webpack-dev-serverは静的出力しないに対応する
...
const jsExcludedModulesRegExp = new RegExp(
  `node_modules\\${path.sep}(?!(${config.transpileNodeModules.join("|")})\\${
    path.sep
  }).*`
);
...
module.exports = (entry) => {
  ...
  const webpackConfig = {
    ...
    // 生成された webpack モジュールとチャンクをキャッシュして、ビルド速度を向上させる
    cache: {
      type: "filesystem",
      buildDependencies: {
        config: [__filename],
    ...
    module: {
      rules: [
        ...
        // ts/tsx
        {
          test: /\.(ts|tsx)$/,
          exclude: jsExcludedModulesRegExp,
          use: [babelLoaderSettings, tsLoaderSettings],
        },
        ...
        // css/sass/scss
        {
          ...
          oneOf: [
            ...
            // for react CSS Module
            {
              use: [
                "style-loader",
                Object.assign({}, cssLoaderSettings, {
